  ...
  React.useEffect(() => {
    const getAnswer = async () => {
      ...
      try {
        ...
        const response = await axios.get(`http://localhost:8000/api/answers/${userId}?last=${true}`, config)
                ...
                {(submissions || []).map((ans, i) => {
                  ...
                  return (
                    <tr key={i}>
                      ...
                      <td>{ans?.value?.suggestion ? <span style={{ color: 'green' }}>{ans?.value?.suggestion}</span> : <span style={{ color: 'red' }}>No suggestion found</span>}</td>
                    </tr>
                  );
                })}
              </table>
